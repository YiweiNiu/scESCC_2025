---
title: "vdj.tcr.STARTRAC.cd4_cd8"
---

```{r knitr, include = FALSE}
DOCNAME = "vdj.tcr.STARTRAC.cd4_cd8"
...
```{r fig.width=6, fig.height=6}
d_4_p %>%
  filter(str_starts(majorCluster, "CD8")) %>%
  ggplot(aes(x = freq, y = expa, color = majorCluster, size = cellNum)) +
  geom_point() +
  scale_color_manual(values = t_level_3_color) +
  geom_text_repel(aes(label = majorCluster), max.overlaps = 20) +
...
```{r fig.width=8, fig.height=3}
dat.plot.tex = out@pIndex.migr %>%
  filter(aid == "scESCC", str_detect(majorCluster, "Tex")) %>%
  dplyr::select(-aid) %>%
  column_to_rownames("majorCluster") %>%
  as.matrix()
yrange = pretty(dat.plot.tex)
col_fun = circlize::colorRamp2(seq(0, max(yrange), length=50),
                               colorRampPalette(rev(brewer.pal(n=7,name="RdBu")))(50),
                               space = "LAB")

Heatmap(dat.plot.tex, col = col_fun, name = "pIndex.migr")
```
