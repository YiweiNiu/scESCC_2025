---
title: "CellChat.byOrigin2.Normal.cellType3"
---

```{r knitr, include = FALSE}
DOCNAME = "CellChat.byOrigin2.Normal.cellType3"
...
srat_sub = subset(seurat, Origin2_n == "Normal")
rm(seurat)
...
```{r netVisual_circle, fig.width=10, fig.height=10}
...
```{r netVisual_circle-each, fig.width=24, fig.height=24}
...
```{r vis-each-signaling-pathway, results='hide'}
...
src_list <- lapply(pathways.show.all, function(cluster){
    src <- c(
        "### {{cluster}}",
        "```{r vis-each-signaling-pathway-{{cluster}}, fig.width=30, fig.height=10}",
...
```{r netAnalysis_computeCentrality, results='hide'}
# Visualize the computed centrality scores using heatmap, allowing ready identification of major signaling roles of cell groups
src_list <- lapply(pathways.show.all, function(cluster){
    src <- c(
        "##### {{cluster}}",
        "```{r netAnalysis_computeCentrality-{{cluster}}, fig.width=20, fig.height=8}",
        "netAnalysis_signalingRole_network(cellchat, signaling = '{{cluster}}', width = 40, height = 5, font.size = 6)",
        "```",
        "")
    #print(src)
    knitr::knit_expand(text = src)
...
```{r netAnalysis_signalingRole_heatmap, fig.width=20, fig.height=20}
# Signaling role analysis on the aggregated cell-cell communication network from all signaling pathways
ht1 <- netAnalysis_signalingRole_heatmap(cellchat, pattern = "outgoing", width = 40, height = 40, font.size = 6)
ht2 <- netAnalysis_signalingRole_heatmap(cellchat, pattern = "incoming", width = 40, height = 40, font.size = 6)
ht1
ht2
```
...
```{r identifyCommunicationPatterns-outgoing, fig.width=8, fig.height=15}
nPatterns = 5
cellchat <- identifyCommunicationPatterns(cellchat, pattern = "outgoing", k = nPatterns,
                                          width = 4, height = 20, font.size = 6)
```

```{r netAnalysis_river-outgoing, fig.height=12}
# river plot
netAnalysis_river(cellchat, pattern = "outgoing")
#> Please make sure you have load `library(ggalluvial)` when running this function
```

```{r netAnalysis_dot-outgoing, fig.width=15, fig.height=10}
# dot plot
netAnalysis_dot(cellchat, pattern = "outgoing", font.size = 8, font.size.title = 10)
```
...
```{r identifyCommunicationPatterns-incoming, fig.width=8, fig.height=15}
nPatterns = 4
cellchat <- identifyCommunicationPatterns(cellchat, pattern = "incoming", k = nPatterns,
                                          width = 4, height = 20, font.size = 6)
```

```{r netAnalysis_river-incoming, fig.height=12}
# river plot
netAnalysis_river(cellchat, pattern = "incoming")
#> Please make sure you have load `library(ggalluvial)` when running this function
```

```{r netAnalysis_dot-incoming, fig.width=15, fig.height=10}
# dot plot
netAnalysis_dot(cellchat, pattern = "incoming", font.size = 8, font.size.title = 10)
```
...
```{r computeNetSimilarity-functional}
...
```{r computeNetSimilarity-structural}
#> Classification learning of the signaling networks for a single dataset
# Visualization in 2D-space
