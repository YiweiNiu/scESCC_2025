---
title: "monocle3.fib.par_2"
---

```{r knitr, include = FALSE}
DOCNAME = "monocle3.fib.par_2"
...
# Load seurat
seurat = readRDS(here::here('output', DOCNAME, 'seurat_fib.par_2.rds'))
seurat
cds = readRDS(here::here('output', DOCNAME, 'monocle3_fib.par_2.rds'))
cds
```
...
# subset: par_2
srat_subset = subset(seurat, subset = level_2 %in% c("pericyte", "myCAF"))
...
saveRDS(srat_subset, file = here::here('output', '04.rm_cells', "seurat_fib.par_2.rds"))

# to h5ad
sceasy::convertFormat(srat_subset, from="seurat", to="anndata",
                      main_layer = 'counts',
                      outFile=here::here('output', DOCNAME, paste0('seurat_fib.par_2', '.h5ad')))
...
# pseudotime
# pericyte-C2 markers
max.avp <- which.max(unlist(FetchData(srat_subset, "MYH11")))
...
data.frame(cell = names(pseudotime(cds)), Pseudotime = pseudotime(cds)) %>%
  write_csv(file = here::here('output/04.rm_cells/monocle3_fib.par_2.pseudotime.csv'))

# save
saveRDS(cds, file = here::here('output', DOCNAME, 'monocle3_fib.par_2.rds'))
```
