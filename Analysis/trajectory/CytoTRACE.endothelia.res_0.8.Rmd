---
title: "CytoTRACE.endothelia.res_0.8"
---
...
DOCNAME = "CytoTRACE.endothelia.res_0.8"
...
seurat = readRDS(here::here('output/04.rm_cells/seurat_endothelia.res_0.8.rds'))
...
```{r CytoTRACE-blood-ec}
seurat_subset = subset(seurat, subset = seurat_clusters %in% c(4, 6), invert = T)
...
meta = FetchData(seurat_subset, vars = c('UMAP_1', 'UMAP_2', 'seurat_clusters')) %>%
  rownames_to_column('barcode')
...
```{r vis-box-CytoTRACE-blood-ec, fig.height=3, fig.width=4}
tibble(barcode = names(result$CytoTRACE),
       value = result$CytoTRACE) %>%
  left_join(meta, by = 'barcode') %>%
  ggplot(aes(x=seurat_clusters, y=value, fill=seurat_clusters)) +
