---
title: "04.sub_cluster.endothelia"
---
...
DOCNAME = "04.sub_cluster.endothelia"
...
src_list <- lapply(resolutions, function(res) {
    src <- c(
        "### Res {{res}} {.unnumbered}",
        "```{r res-markers-dot-{{res}}, fig.width=10, fig.height=10}",
...
```{r classify-endo, fig.width=8, fig.height=6}
FeaturePlot(seurat, features = c("FLT1", "PDPN", "PROX1"), ncol = 2)
```

## EndMT

```{r EndMT-dot, fig.height=8, fig.width=10}
EndMT.markers = c('SMN1', 'SNAI1', 'SNAI2', 'TWIST1', 'NFATC1', 'MMP2', 'COL16A1', 'COL3A1', 'TGFB1', 'TGFB2', 'TGFB3', 'CDH5', 'PTPRC')
DotPlot(seurat, features = rev(unlist(EndMT.markers)), dot.scale = 4, group.by = 'seurat_clusters') + RotatedAxis() + theme(axis.text.x = element_text(size = rel(0.7)), axis.text.y = element_text(size = rel(0.9)), panel.grid = element_line(color = 'grey'), legend.position = 'bottom', legend.box = 'vertical')
```

```{r EndMT-scatter, fig.height=14, fig.width=12}
FeaturePlot(seurat, features = EndMT.markers, ncol = 3)
```
